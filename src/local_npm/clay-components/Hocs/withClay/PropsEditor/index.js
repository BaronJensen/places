...
const getSlot = (slot, type, id) =>{

	if(!slot[type][id])
	return slot["default"][id]

return {...slot["default"][id], ...slot[type][id]}
...
export const PropsEditor = (props)=>{
	...
	const componentStyle = getSlot(data[props.config.type], slots[props.config.type], props.id)
	...
	return (
		<React.Fragment>
			...
			<Container
				...
				>Slot: {slots[config.type]} <BaselineArrowDropDown/> <br/> <br/>Component: {config.type}
				</Typography>
				...
				{config.schema.map((items, id)=>{


					return (
					<React.Fragment key={"editor_good" + id}>
					...
					width="90%"
					marginB="20"
					css="width:90%; margin: 20px auto;">{items.title}
					</Typography>
